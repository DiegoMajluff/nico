# 🛠️ Instalación y Compilación de Nico v1.1.1
...
```bash
cd /ruta/a/tu/carpeta/nico-v1.1.1
```
*(Reemplazá `/ruta/a/tu/carpeta/` por la ruta real. Si usás Ubuntu, suele ser `~/Descargas/nico-v1.1.1` o similar).*
...
4. Abrí una terminal en esa carpeta (hacé clic derecho en un espacio vacío → "Abrir en Windows Terminal" o `cmd`), y escribí:
   ```cmd
   nico.exe ..\..\ejemplos\HolaNico.nico
   ```
...
2. Escribí `MSYS2`.
3. Verás varias opciones. Hacé clic **SOLO** en **"UCRT64"** (tiene un icono de terminal negra).
> 🚨 **Muy importante:** No abras "MSYS", "MINGW64" ni "CLANG64". Solo **"UCRT64"**. Si abrís otra, los comandos de más abajo fallarán.
...
En la misma terminal UCRT64, escribí:
```bash
pacman -S --needed base-devel mingw-w64-ucrt-x86_64-toolchain mingw-w64-ucrt-x86_64-sqlite3
```
...
Si dice `command not found`, volvé al Paso 2 y aseguráte de estar en la terminal **UCRT64**.

#### Paso 6: Compilar Nico
1. En la terminal, navegá a la carpeta de tu proyecto. Ejemplo:
   ```bash
   cd /c/Users/TuNombre/nico-v1.1.1
   ```
...
- `libsqlite3-0.dll`
- `libwinpthread-1.dll`
*(Los encontrás en `C:\msys64\ucrt64\bin\` y en la carpeta `bin\windows\` del proyecto. Sin ellos, Windows te mostrará un error de "falta DLL").*
