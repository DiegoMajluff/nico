...
// ============================================================
// FUNCIÓN PARA EVALUAR EXPRESIONES EN EL MOTOR LÓGICO
// ============================================================
double evaluar_expresion_logica(const char *expr, Bindings *bindings, void *contexto_ptr)
{
    (void)contexto_ptr;
    ...
    // 1. ¿Es una variable pura?
    bool es_variable_pura = isupper((unsigned char)expr[0]);
    if (es_variable_pura)
    {
        for (const char *p = expr; *p; p++)
        {
            if (*p == ' ' || *p == '+' || *p == '-' || *p == '*' || *p == '/' ||
                *p == '%' || *p == '^' || *p == '(' || *p == ')')
            {
                es_variable_pura = false;
                break;
            }
        }
    }

    if (es_variable_pura && bindings)
    {
        for (int i = 0; i < bindings->count; i++)
        {
            if (strcmp(bindings->items[i].variable, expr) == 0)
            {
                char *endptr;
                double val = strtod(bindings->items[i].valor, &endptr);
                if (endptr != bindings->items[i].valor && *endptr == '\0')
                    return val;
                return 0.0;
            }
        }
    ...
    // 2. ¿Es un número literal?
    char *endptr;
    double val = strtod(expr, &endptr);
    if (endptr != expr && *endptr == '\0')
        return val;

    // 3. Es una expresión aritmética - delegar a evaluar_expresion_simple de logica.c
    return evaluar_expresion_simple(expr, bindings);
}
...
static Valor evaluar_operador_binario(OperadorBinario op, Valor izq, Valor der, Contexto *ctx)
{
    ...
    case OP_Y_LOGICO:
    {
        double v_izq = (izq.tipo == VALOR_ENTERO) ? (double)izq.datos.entero : (izq.tipo == VALOR_ENTERO_SIN_SIGNO) ? (double)izq.datos.entero_sin_signo
                                                                           : (izq.tipo == VALOR_DECIMAL)            ? izq.datos.decimal
                                                                           : (izq.tipo == VALOR_LOGICA)             ? (izq.datos.logica ? 1.0 : 0.0)
                                                                                                                    : 0.0;
        double v_der = (der.tipo == VALOR_ENTERO) ? (double)der.datos.entero : (der.tipo == VALOR_ENTERO_SIN_SIGNO) ? (double)der.datos.entero_sin_signo
                                                                           : (der.tipo == VALOR_DECIMAL)            ? der.datos.decimal
                                                                           : (der.tipo == VALOR_LOGICA)             ? (der.datos.logica ? 1.0 : 0.0)
                                                                                                                    : 0.0;
...
{
    if (!nodo || ctx->hay_error)
        return valor_crear_vacio();

    // CHEQUEO DE INTERRUPCIÓN (Ctrl+C)
    if (hay_interrupcion())
    {
        resetear_interrupcion(); // Limpiamos la bandera para que ATRAPAR pueda funcionar después
    ...
    case AST_POSICIONAR:
    {
        NodoAST *fila_nodo = (nodo->tipo == AST_CURSOR) ? nodo->datos.cursor.fila : nodo->datos.posicionar.fila;
        NodoAST *col_nodo = (nodo->tipo == AST_CURSOR) ? nodo->datos.cursor.columna : nodo->datos.posicionar.columna;
    ...
    {
        const char *nombre = (nodo->tipo == AST_ANCHOTERMINAL) ? nodo->datos.anchoterminal.variable : nodo->datos.altoterminal.variable;
        ...
        {
            valor = (nodo->tipo == AST_ANCHOTERMINAL) ? (csbi.srWindow.Right - csbi.srWindow.Left + 1) : (csbi.srWindow.Bottom - csbi.srWindow.Top + 1);
        }
#else
        struct winsize ws;
    ...
    case AST_ESPERAR:
    {
        ...
        double val = (cantidad.tipo == VALOR_ENTERO) ? (double)cantidad.datos.entero : cantidad.datos.decimal;
    ...
    {
        const char *var = (nodo->tipo == AST_ABRIRARCHIVO) ? nodo->datos.abrirarchivo.variable : nodo->datos.usararchivo.variable;
        const char *ruta = (nodo->tipo == AST_ABRIRARCHIVO) ? nodo->datos.abrirarchivo.ruta : nodo->datos.usararchivo.ruta;
        int modo = (nodo->tipo == AST_ABRIRARCHIVO) ? nodo->datos.abrirarchivo.modo : nodo->datos.usararchivo.modo;
    ...
    case AST_ESCRIBIRARCHIVO:
    {
        ...
        {
            char buffer[256];
            if (contenido.tipo == VALOR_ENTERO)
                snprintf(buffer, sizeof(buffer), "%lld", contenido.datos.entero);
            else if (contenido.tipo == VALOR_DECIMAL)
                snprintf(buffer, sizeof(buffer), "%g", contenido.datos.decimal);
            else
                buffer[0] = '\0';
            fprintf(v->datos.archivo, "%s\n", buffer);
        }
    ...
    case AST_LLAMADA_FUNCION:
    {
        ...
        if (func)
        {
            ...
            TablaSimbolos *tabla_local = NULL;
            ...
            // USAR POOL DE TABLAS PARA EVITAR MALLOC EN RECURSIÓN
            ...
            // 3. INICIALIZAR LA TABLA LOCAL
            tabla_local->primera = NULL;
            tabla_local->padre = tabla_anterior; 
        ...
        limpiar_funcion_usuario:
            ...
            // LIMPIEZA SEGURA DE LA TABLA LOCAL
            if (tabla_local != NULL)
            {
                // Limpiar símbolos
                ...
                // Verificar si esta tabla pertenece al pool
                bool en_pool = false;
                for (int i = 0; i < ctx->pool_index; i++)
                {
                    if (ctx->pool_tablas[i] == tabla_local)
                    {
                        en_pool = true;
                        break;
                    }
            ...
            // Destruir argumentos (fuera del goto para que siempre se ejecute)
            for (int i = 0; i < num_args; i++)
            {
    ...
    case AST_DECLARACION_TEXTO_EXTENSO:
    {
        ...
        Valor valor = valor_crear_texto(
            nodo->datos.declaracion_texto_extenso.valor_inicial ? nodo->datos.declaracion_texto_extenso.valor_inicial : "");
    ...
    case AST_PARA:
    {
        ...
        if (nodo->datos.para.paso)
        {
            ...
            if (ctx->hay_error)
                return valor_crear_vacio();
            if (v_paso.tipo == VALOR_ENTERO)
                paso = v_paso.datos.entero;
            else if (v_paso.tipo == VALOR_DECIMAL)
                paso = (int)v_paso.datos.decimal;
        }
    ...
    {
        const char *nombre = (nodo->tipo == AST_HORAACTUAL) ? nodo->datos.horaactual.variable : nodo->datos.fechaactual.variable;
    ...
    case AST_LLAMAR_A:
    {
        ...
        if (ctx->profundidad_llamada > 1000)
        {
        ...
        // DECLARAR tabla_local ANTES de usarla
        TablaSimbolos *tabla_local = NULL;
        ...
        tabla_local->primera = NULL;
        tabla_local->padre = tabla_anterior; // CORRECCIÓN CLAVE: encadenar al scope actual
        ctx->tabla_actual = tabla_local;
        ctx->estado_flujo = FLUJO_NORMAL;
        ctx->valor_retorno = valor_crear_vacio();
        ctx->profundidad_llamada++;
    ...
    // ============================================================
    case AST_AGREGARHECHO:
    {
        Valor val_expr;
        val_expr = evaluar_nodo(nodo->datos.logica_comando.expresion, ctx);
        if (val_expr.tipo == VALOR_TEXTO && ctx->logica)
        {
            bc_agregar_hecho_desde_string(ctx->logica, val_expr.datos.texto);
        }
        valor_destruir(&val_expr);
        return valor_crear_vacio();
    }

    case AST_AGREGARREGLA:
    {
        Valor val_expr;
        val_expr = evaluar_nodo(nodo->datos.logica_comando.expresion, ctx);
        if (val_expr.tipo == VALOR_TEXTO && ctx->logica)
        {
            bc_agregar_regla_desde_string(ctx->logica, val_expr.datos.texto);
        }
        valor_destruir(&val_expr);
        return valor_crear_vacio();
    }

    case AST_CONSULTAR_LOGICA:
    {
        Valor val_expr;
        val_expr = evaluar_nodo(nodo->datos.logica_comando.expresion, ctx);
        if (val_expr.tipo == VALOR_TEXTO && ctx->logica)
        {
            // USAR bc_consultar para obtener los bindings y pasar el contexto
            ResultadoConsulta resultado = bc_consultar(ctx->logica, val_expr.datos.texto, ctx);
            if (resultado.exito)
            {
                // Copiar bindings a la tabla de símbolos (para usar en SI imperativo)
                if (resultado.num_soluciones >= 1)
                {
                    for (int i = 0; i < resultado.soluciones[0].count; i++)
                    {
                        Binding *b = &resultado.soluciones[0].items[i];
                        if (!strchr(b->variable, '_')) // Solo variables originales
                        {
                            char *endptr;
                            double val_num = strtod(b->valor, &endptr);
                            if (endptr != b->valor && *endptr == '\0')
                            {
                                if (val_num == (long long)val_num)
                                {
                                    tabla_simbolos_definir(ctx->tabla_actual, b->variable,
                                                           valor_crear_entero((long long)val_num), false);
                                }
                                else
                                {
                                    tabla_simbolos_definir(ctx->tabla_actual, b->variable,
                                                           valor_crear_decimal(val_num), false);
                                }
                            }
                            else
                            {
                                tabla_simbolos_definir(ctx->tabla_actual, b->variable,
                                                       valor_crear_texto(b->valor), false);
                            }
                        }
                    }
                }

                // === IMPRIMIR RESULTADOS DIRECTAMENTE (sin llamar a bc_imprimir_soluciones) ===
                // Extraer variables originales de la consulta
                ...
                const char *inicio = val_expr.datos.texto;
                const char *p = val_expr.datos.texto;
                while (*inicio == ' ')
                    inicio++;
                ...
                // Recolectar variables (usando la función recursiva)
                // Recolectar variables (usando la función recursiva que busca DENTRO de listas)
                for (int c = 0; c < num_condiciones; c++)
                {
                    char predicado[64];
                    char args[MAX_ARGS][64];
                    int num_args = 0;
                    if (parsear_predicado_publico(condiciones[c], predicado, args, &num_args))
                    {
                        for (int i = 0; i < num_args; i++)
                        {
                            recolectar_variables_en_arg_publico(args[i], vars_originales, &num_vars);
                        }
                    }
                }
                ...
                else
                {
                    for (int s = 0; s < resultado.num_soluciones; s++)
                    {
                        ...
                        {
                            const char *valor = resolver_var_publico(&resultado.soluciones[s], vars_originales[i]);
    ...
    case AST_LISTARHECHOS:
    {
        if (ctx->logica)
        {
            bc_imprimir(ctx->logica);
        }
        return valor_crear_vacio();
    }

    case AST_LISTARREGLAS:
    {
        if (ctx->logica)
        {
            bc_imprimir(ctx->logica);
    ...
    case AST_ENCONTRARTODOS:
    {
        // BUSCARTODOS("variable", "consulta", $resultado)
        // Recolecta todos los valores de 'variable' en las soluciones de 'consulta'
        // y los guarda como lista en $resultado

        if (!ctx->logica)
        {
            contexto_set_error(ctx, "Motor lógico no inicializado");
            return valor_crear_vacio();
        }

        // 1. Evaluar la consulta (debe ser un string)
        Valor val_consulta = evaluar_nodo(nodo->datos.encontrartodos.consulta, ctx);
        if (ctx->hay_error)
            return valor_crear_vacio();

        if (val_consulta.tipo != VALOR_TEXTO)
        {
            contexto_set_error(ctx, "BUSCARTODOS: la consulta debe ser un string");
            valor_destruir(&val_consulta);
            return valor_crear_vacio();
        }

        const char *consulta_str = val_consulta.datos.texto;
        const char *variable_buscar = nodo->datos.encontrartodos.variable;
        const char *resultado_var = nodo->datos.encontrartodos.resultado;

        // 2. Ejecutar la consulta
        ResultadoConsulta resultado = bc_consultar(ctx->logica, consulta_str, ctx);

        if (!resultado.exito)
        {
            // No hay soluciones, crear lista vacía
            Valor lista_vacia;
            lista_vacia.tipo = VALOR_LISTA;
            lista_vacia.tamano = 0;
            lista_vacia.filas = 0;
            lista_vacia.columnas = 0;
            lista_vacia.tipo_elemento = TIPO_TEXTO;
            lista_vacia.datos.lista = NULL;

            tabla_simbolos_definir(ctx->tabla_actual, resultado_var, lista_vacia, false);
            valor_destruir(&val_consulta);
            return valor_crear_vacio();
        }

        // 3. Recolectar valores de la variable en cada solución
        // Primero, contar cuántas soluciones únicas hay para la variable
        char *valores_recolectados[MAX_SOLUCIONES];
        int num_valores = 0;

        for (int s = 0; s < resultado.num_soluciones; s++)
        {
            // Resolver la variable en esta solución
            const char *valor_resuelto = resolver_var_publico(&resultado.soluciones[s], variable_buscar);

            // Verificar si ya está en la lista (evitar duplicados)
            bool ya_existe = false;
            for (int i = 0; i < num_valores; i++)
            {
                if (strcmp(valores_recolectados[i], valor_resuelto) == 0)
                {
                    ya_existe = true;
                    break;
                }
            ...
            if (!ya_existe && num_valores < MAX_SOLUCIONES)
            {
                valores_recolectados[num_valores] = strdup(valor_resuelto);
                num_valores++;
            }
        }

        // 4. Construir la lista de Nico
        Valor lista_resultado;
        lista_resultado.tipo = VALOR_LISTA;
        lista_resultado.tamano = num_valores;
        lista_resultado.filas = 0;
        lista_resultado.columnas = 0;
        lista_resultado.tipo_elemento = TIPO_TEXTO;

        if (num_valores > 0)
        {
            lista_resultado.datos.lista = malloc(sizeof(Valor) * num_valores);
            if (!lista_resultado.datos.lista)
            {
                contexto_set_error(ctx, "BUSCARTODOS: no se pudo asignar memoria para la lista");
                for (int i = 0; i < num_valores; i++)
                    free(valores_recolectados[i]);
                valor_destruir(&val_consulta);
                return valor_crear_vacio();
            }
            ...
            for (int i = 0; i < num_valores; i++)
            {
                lista_resultado.datos.lista[i] = valor_crear_texto(valores_recolectados[i]);
                free(valores_recolectados[i]); // Liberar el string duplicado
            }
        }
        else
        {
            lista_resultado.datos.lista = NULL;
        }

        // 5. Guardar la lista en la variable destino
        tabla_simbolos_definir(ctx->tabla_actual, resultado_var, lista_resultado, false);
