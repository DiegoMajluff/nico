...
typedef struct NodoAST {
    ...
    union {
        ...
        // Motor lógico - AGREGARHECHO, AGREGARREGLA, CONSULTAR
        struct
        {
            struct NodoAST *expresion; // String con la sintaxis lógica
        } logica_comando;

        // Motor lógico - ENCONTRARTODOS
        struct
        {
            char *variable;           // Nombre de la variable a recolectar (ej: "X")
            struct NodoAST *consulta; // String con la consulta
            char *resultado;          // Nombre de la variable destino (sin $)
        } encontrartodos;   

        // Motor lógico - LIMPIARLOGICA, LISTARHECHOS, LISTARREGLAS
        struct
        {
            int dummy;
        } logica_simple;
...
// Funciones de creación de nodos lógicos
NodoAST* crear_nodo_hecho_logico(const char* predicado, NodoAST** argumentos, 
                                  int num_argumentos, int linea);
NodoAST* crear_nodo_regla_logica(const char* nombre, NodoAST** cabeza, int num_cabeza,
                                  NodoAST** cuerpo, int num_cuerpo, int linea);
NodoAST* crear_nodo_consulta_logica(const char* predicado, NodoAST** argumentos,
                                     int num_argumentos, const char* variable_resultado, int linea);
...
// FUNCIONES DE CREACIÓN DE NODOS DEL MOTOR LÓGICO
NodoAST *crear_nodo_agregarhecho(NodoAST *expresion, int linea);
NodoAST *crear_nodo_agregarregla(NodoAST *expresion, int linea);
