...
// ============================================================
void parser_set_error(Parser *parser, const char *mensaje)
{
    // Si ya hay un error, NO lo sobrescribimos. Preservamos el primero.
    if (parser->hay_error)
    {
        return;
    }
...
static NodoAST *parsear_si(Parser *parser)
{
    ...
    // 1. Paréntesis de apertura OBLIGATORIO
    if (parser->token_actual.tipo != TOK_PARENTESIS_ABRIR)
    {
        error_msg = "Se esperaba '(' después de SI. Sintaxis correcta: SI(condición) ENTONCES";
    }
    else
    {
        parser_avanzar(parser); // Consumir '('
    ...
    // Guardar ubicación ANTES de parsear la expresión
    int antes_exp_linea = parser->token_actual.linea;
    int antes_exp_columna = parser->token_actual.columna;

    // 2. Expresión de condición
    NodoAST *condicion = parsear_expresion(parser);

    // 3. Paréntesis de cierre
    if (parser->token_actual.tipo != TOK_PARENTESIS_CERRAR)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba ')' para cerrar la condición del SI";
            error_linea = antes_exp_linea;
            error_columna = antes_exp_columna;
        }
    }
    else
    {
    ...
    // 4. ENTONCES
    if (parser->token_actual.tipo != TOK_ENTONCES)
    {
        if (!error_msg)
            error_msg = "Se esperaba la palabra clave ENTONCES";
    }
    else
    {
        parser_avanzar(parser); // Consumir ENTONCES
    }
    ...
    // 6. Parsear SINO SI o SINOSI
    // Solo tratar como SINOSI si están en la MISMA línea.
    // Si están en líneas distintas, el SINO pertenece a un bloque y el SI es una sentencia anidada.
    while (parser->token_actual.tipo == TOK_SINOSI ||
           (parser->token_actual.tipo == TOK_SINO && 
            parser->token_siguiente.tipo == TOK_SI &&
            parser->token_actual.linea == parser->token_siguiente.linea))
    {
        ...
        {
            if (!error_msg)
                error_msg = "Se esperaba '(' después de SINO SI / SINOSI";
        }
        else
        {
            parser_avanzar(parser);
        ...
        {
            if (!error_msg)
                error_msg = "Se esperaba ')' para cerrar la condición del SINO SI / SINOSI";
        }
        else
        {
            parser_avanzar(parser);
    ...
    {
        if (!error_msg)
            error_msg = "Se esperaba FIN para cerrar el bloque SI";
    }
    else
    {
        parser_avanzar(parser);
    ...
    // 9. SI (cierre de la estructura)
    if (parser->token_actual.tipo != TOK_SI)
    {
        if (!error_msg)
            error_msg = "Se esperaba SI para cerrar la estructura";
    }
    else
    {
        parser_avanzar(parser);
    ...
    // 10. Reportar error diferido si existe
    if (error_msg)
    {
        parser->hay_error = true;
        snprintf(parser->mensaje_error, sizeof(parser->mensaje_error),
                 "Línea %d, columna %d: %s", error_linea, error_columna, error_msg);
        return NULL;
    }
...
static NodoAST *parsear_para(Parser *parser)
{
    ...
    bool falta_parentesis_apertura = false;
    char *error_msg = NULL;
    int error_linea = 0;
    int error_columna = 0;
    ...
    if (parser->token_actual.tipo != TOK_PARENTESIS_ABRIR)
    {
        falta_parentesis_apertura = true;
        error_msg = "Se esperaba '(' después de PARA. Sintaxis correcta: PARA($var DESDE inicio HASTA fin) HACER";
    }
    else
    {
        parser_avanzar(parser); // Consumir '('
    ...
    if (parser->token_actual.tipo != TOK_VARIABLE)
    {
        parser_set_error(parser, "Se esperaba variable de control (ej: $i)");
        return NULL;
    }
    char *variable = strdup(parser->token_actual.valor + 1);
    parser_avanzar(parser);

    if (parser->token_actual.tipo == TOK_DESDE || parser->token_actual.tipo == TOK_IGUAL)
    {
        parser_avanzar(parser);
    }
    ...
    if (parser->token_actual.tipo != TOK_HASTA)
    {
        parser_set_error(parser, "Se esperaba la palabra clave HASTA");
        free(variable);
        return NULL;
    }
    parser_avanzar(parser);

    // Guardar ubicación ANTES de parsear la expresión de fin
    int antes_fin_linea = parser->token_actual.linea;
    int antes_fin_columna = parser->token_actual.columna;
    ...
    NodoAST *paso = NULL;
    if (parser->token_actual.tipo == TOK_PASO)
    {
        parser_avanzar(parser);
    ...
    if (parser->token_actual.tipo != TOK_PARENTESIS_CERRAR)
    {
        if (!falta_parentesis_apertura)
        {
            error_msg = "Se esperaba ')' para cerrar la condición del PARA";
            error_linea = antes_fin_linea;
            error_columna = antes_fin_columna;
        }
    }
    else
    {
    ...
    if (parser->token_actual.tipo != TOK_HACER)
    {
        parser_set_error(parser, "Se esperaba la palabra clave HACER");
        free(variable);
        return NULL;
    }
    parser_avanzar(parser);
    ...
    if (parser->token_actual.tipo != TOK_FIN)
    {
        parser_set_error(parser, "Se esperaba FIN para cerrar el bloque PARA");
        free(variable);
        return NULL;
    }
    parser_avanzar(parser);

    if (parser->token_actual.tipo != TOK_PARA)
    {
        parser_set_error(parser, "Se esperaba PARA para cerrar la estructura");
        free(variable);
        return NULL;
    }
    parser_avanzar(parser);
...
static NodoAST *parsear_mientras(Parser *parser)
{
    ...
    if (parser->token_actual.tipo != TOK_PARENTESIS_ABRIR)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba '(' después de MIENTRAS. Sintaxis correcta: MIENTRAS(condición) HACER";
            error_linea = parser->token_actual.linea;
            error_columna = parser->token_actual.columna;
        }
    }
    else
    {
    ...
    if (parser->token_actual.tipo != TOK_PARENTESIS_CERRAR)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba ')' para cerrar la condición del MIENTRAS";
            error_linea = antes_exp_linea;
            error_columna = antes_exp_columna;
        }
    }
    ...
    if (parser->token_actual.tipo != TOK_HACER)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba la palabra clave HACER";
            error_linea = parser->token_actual.linea;
            error_columna = parser->token_actual.columna;
        }
    }
    ...
    if (parser->token_actual.tipo != TOK_FIN)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba FIN para cerrar el bloque MIENTRAS";
            error_linea = parser->token_actual.linea;
            error_columna = parser->token_actual.columna;
        }
    }
    ...
    if (parser->token_actual.tipo != TOK_MIENTRAS)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba MIENTRAS para cerrar la estructura";
            error_linea = parser->token_actual.linea;
            error_columna = parser->token_actual.columna;
        }
    }
    ...
    // Reportar error diferido con la ubicación ORIGINAL
    if (error_msg)
    {
        parser->hay_error = true;
        snprintf(parser->mensaje_error, sizeof(parser->mensaje_error),
...
static NodoAST *parsear_realizar(Parser *parser)
{
    ...
    // 1. Parsear bloque hasta encontrar MIENTRAS
    NodoAST *bloque = parsear_bloque_hasta(parser, TOK_MIENTRAS);

    // 2. MIENTRAS (Obligatorio para cerrar)
    if (parser->token_actual.tipo != TOK_MIENTRAS)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba la palabra clave MIENTRAS para cerrar el bucle REALIZAR";
            error_linea = parser->token_actual.linea;
            error_columna = parser->token_actual.columna;
        }
    }
    ...
    // 3. Paréntesis de apertura OBLIGATORIO
    if (parser->token_actual.tipo != TOK_PARENTESIS_ABRIR)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba '(' después de MIENTRAS. Sintaxis correcta: MIENTRAS(condición)";
            error_linea = parser->token_actual.linea;
            error_columna = parser->token_actual.columna;
        }
    }
    ...
    if (parser->token_actual.tipo != TOK_PARENTESIS_CERRAR)
    {
        ...
        {
            error_msg = "Se esperaba ')' para cerrar la condición del MIENTRAS";
            // Usar la ubicación ANTES de la expresión, no la actual
            error_linea = antes_exp_linea;
            error_columna = antes_exp_columna;
        }
    }
    ...
    // 6. Reportar error diferido con la ubicación ORIGINAL
    if (error_msg)
    {
        parser->hay_error = true;
        snprintf(parser->mensaje_error, sizeof(parser->mensaje_error),
...
static NodoAST *parsear_segun_caso(Parser *parser)
{
    ...
    // 1. Esperar CASO
    if (parser->token_actual.tipo != TOK_CASO)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba la palabra clave CASO después de SEGUN";
            error_linea = parser->token_actual.linea;
            error_columna = parser->token_actual.columna;
        }
    }
    ...
    if (parser->token_actual.tipo != TOK_PARENTESIS_ABRIR)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba '(' después de CASO";
            error_linea = parser->token_actual.linea;
            error_columna = parser->token_actual.columna;
        }
    }
    ...
    if (parser->token_actual.tipo != TOK_PARENTESIS_CERRAR)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba ')' para cerrar la expresión del SEGUN";
            error_linea = antes_exp_linea;
            error_columna = antes_exp_columna;
        }
    }
    ...
    // 5. Parsear casos hasta encontrar FIN o EOF
           ...
           parser->token_actual.tipo != TOK_EOF)
    {
        if (parser->token_actual.tipo == TOK_CASO)
        {
            ...
            if (parser->token_actual.tipo != TOK_NUMERO_ENTERO)
            {
                if (!error_msg)
                {
                    error_msg = "Se esperaba un valor numérico entero después de CASO";
                    error_linea = parser->token_actual.linea;
                    error_columna = parser->token_actual.columna;
                }
                break;
            ...
            // Esperar CORTE al final del caso
            if (parser->token_actual.tipo != TOK_CORTE)
            {
                if (!error_msg)
                {
                    error_msg = "Se esperaba CORTE al final del caso";
                    error_linea = parser->token_actual.linea;
                    error_columna = parser->token_actual.columna;
                }
                break;
            }
            parser_avanzar(parser); // Consumir CORTE

            NodoAST *nodo_caso = crear_nodo_caso(valor, bloque_caso, linea_caso);
            agregar_sentencia_a_bloque(casos, nodo_caso);
        }
        else if (parser->token_actual.tipo == TOK_POR)
        {
            int linea_defecto = parser->token_actual.linea;
            parser_avanzar(parser); // Consumir POR

            if (parser->token_actual.tipo != TOK_DEFECTO)
            {
                if (!error_msg)
                {
                    error_msg = "Se esperaba la palabra clave DEFECTO después de POR";
                    error_linea = parser->token_actual.linea;
                    error_columna = parser->token_actual.columna;
                }
                break;
            ...
            parser_avanzar(parser); // Consumir DEFECTO
            NodoAST *bloque_defecto = parsear_bloque(parser);

            // Esperar CORTE al final del caso por defecto
            if (parser->token_actual.tipo != TOK_CORTE)
            {
                if (!error_msg)
                {
                    error_msg = "Se esperaba CORTE al final del caso POR DEFECTO";
                    error_linea = parser->token_actual.linea;
                    error_columna = parser->token_actual.columna;
                }
                break;
        ...
        else
        {
            if (!error_msg)
            {
                error_msg = "Se esperaba CASO o POR DEFECTO dentro del bloque SEGUN";
                error_linea = parser->token_actual.linea;
                error_columna = parser->token_actual.columna;
            }
            break;
    ...
    if (parser->token_actual.tipo != TOK_FIN)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba FIN para cerrar la estructura SEGUN";
            error_linea = parser->token_actual.linea;
            error_columna = parser->token_actual.columna;
        }
    }
    ...
    // 7. Cierre: SEGUN
    if (parser->token_actual.tipo != TOK_SEGUN)
    {
        if (!error_msg)
        {
            error_msg = "Se esperaba SEGUN para cerrar la estructura";
            error_linea = parser->token_actual.linea;
            error_columna = parser->token_actual.columna;
        }
    }
    ...
    // 8. Reportar error diferido CON UBICACIÓN PRECISA
    if (error_msg)
    {
        parser->hay_error = true;
        snprintf(parser->mensaje_error, sizeof(parser->mensaje_error),
...
static NodoAST* parsear_sentencia(Parser* parser) {
    ...
    // AGREGARHECHO("predicado(arg1, arg2)")
    if (parser_coincidir(parser, TOK_AGREGARHECHO))
    {
        int linea = parser->token_actual.linea;
        parser_avanzar(parser);
        if (!parser_esperar(parser, TOK_PARENTESIS_ABRIR))
        ...
        if (!parser_coincidir(parser, TOK_TEXTO))
        {
            parser_set_error(parser, "Se esperaba una cadena con el hecho entre comillas");
            return NULL;
        }
        char *hecho = strdup(parser->token_actual.valor);
        parser_avanzar(parser);
        if (!parser_esperar(parser, TOK_PARENTESIS_CERRAR))
        {
            free(hecho);
        ...
        NodoAST *expr = crear_nodo_literal_texto(hecho, linea);
        free(hecho);
        return crear_nodo_agregarhecho(expr, linea);
    }

    // AGREGARREGLA("nombre(X, Y) SI predicado1(X, Z), predicado2(Z, Y)")
    if (parser_coincidir(parser, TOK_AGREGARREGLA))
    {
        int linea = parser->token_actual.linea;
        parser_avanzar(parser);
        if (!parser_esperar(parser, TOK_PARENTESIS_ABRIR))
        ...
        if (!parser_coincidir(parser, TOK_TEXTO))
        {
            parser_set_error(parser, "Se esperaba una cadena con la regla entre comillas");
            return NULL;
        }
        char *regla = strdup(parser->token_actual.valor);
        parser_avanzar(parser);
        if (!parser_esperar(parser, TOK_PARENTESIS_CERRAR))
        {
            free(regla);
        ...
        NodoAST *expr = crear_nodo_literal_texto(regla, linea);
        free(regla);
        return crear_nodo_agregarregla(expr, linea);
    }

    // CONSULTAR("predicado(arg1, X)")
    if (parser_coincidir(parser, TOK_CONSULTAR))
    {
        int linea = parser->token_actual.linea;
        parser_avanzar(parser);
        if (!parser_esperar(parser, TOK_PARENTESIS_ABRIR))
        ...
        if (!parser_coincidir(parser, TOK_TEXTO))
        {
            parser_set_error(parser, "Se esperaba una cadena con la consulta entre comillas");
            return NULL;
        }
        char *consulta = strdup(parser->token_actual.valor);
        parser_avanzar(parser);
        if (!parser_esperar(parser, TOK_PARENTESIS_CERRAR))
        {
            free(consulta);
        ...
        NodoAST *expr = crear_nodo_literal_texto(consulta, linea);
        free(consulta);
        return crear_nodo_consultar(expr, linea);
    }

    // LIMPIARLOGICA (con o sin paréntesis)
    if (parser_coincidir(parser, TOK_LIMPIARLOGICA))
    {
        int linea = parser->token_actual.linea;
        parser_avanzar(parser);
        ...
        return crear_nodo_limpiarlogica(linea);
    }

    // LISTARHECHOS (con o sin paréntesis)
    if (parser_coincidir(parser, TOK_LISTARHECHOS))
    {
        int linea = parser->token_actual.linea;
        parser_avanzar(parser);
        // Paréntesis opcionales
        ...
        return crear_nodo_listarhechos(linea);
    }

    // LISTARREGLAS (con o sin paréntesis)
    if (parser_coincidir(parser, TOK_LISTARREGLAS))
    {
        int linea = parser->token_actual.linea;
        parser_avanzar(parser);
        // Paréntesis opcionales
        ...
        return crear_nodo_listarreglas(linea);
    }

    // ENCONTRARTODOS("variable", "consulta", $resultado)
    if (parser_coincidir(parser, TOK_BUSCARTODOS))
    {
        int linea = parser->token_actual.linea;
        parser_avanzar(parser); // Consumir ENCONTRARTODOS

        if (!parser_esperar(parser, TOK_PARENTESIS_ABRIR))
            return NULL;

        // Primer argumento: nombre de la variable a recolectar (string)
        if (!parser_coincidir(parser, TOK_TEXTO))
        {
            parser_set_error(parser, "Se esperaba una cadena con el nombre de la variable");
            return NULL;
        }
        char *variable = strdup(parser->token_actual.valor);
        parser_avanzar(parser);
        ...
        // Segundo argumento: consulta (string)
        if (!parser_coincidir(parser, TOK_TEXTO))
        {
            parser_set_error(parser, "Se esperaba una cadena con la consulta");
            free(variable);
            return NULL;
        }
        char *consulta_str = strdup(parser->token_actual.valor);
        parser_avanzar(parser);
        NodoAST *consulta = crear_nodo_literal_texto(consulta_str, linea);
        free(consulta_str);

        if (!parser_esperar(parser, TOK_COMA))
        {
            free(variable);
            liberar_nodo(consulta);
            return NULL;
        }

        // Tercer argumento: variable destino (con $)
        if (!parser_coincidir(parser, TOK_VARIABLE))
        {
            parser_set_error(parser, "Se esperaba variable destino (con $)");
            free(variable);
            liberar_nodo(consulta);
            return NULL;
        }
        char *resultado = strdup(parser->token_actual.valor + 1); // Sin el $
        parser_avanzar(parser);
        ...
        NodoAST *nodo = crear_nodo_encontrartodos(variable, consulta, resultado, linea);
        free(variable);
        free(resultado);
        return nodo;
    }
...
static NodoAST *parsear_bloque(Parser *parser)
{
    ...
    while (!parser->hay_error &&
           ...
           parser->token_actual.tipo != TOK_CORTE)
...
static NodoAST *parsear_bloque_hasta(Parser *parser, TipoToken token_parada)
{
    ...
    while (!parser->hay_error &&
           ...
           !parser_coincidir(parser, TOK_CORTE) &&
