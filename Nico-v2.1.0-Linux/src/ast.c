...
// ============================================================
// CREACIÓN DE NODOS DEL MOTOR LÓGICO
// ============================================================
NodoAST *crear_nodo_agregarhecho(NodoAST *expresion, int linea)
{
    NodoAST *nodo = crear_nodo_base(AST_AGREGARHECHO, linea);
    nodo->datos.logica_comando.expresion = expresion;
    return nodo;
}

NodoAST *crear_nodo_agregarregla(NodoAST *expresion, int linea)
{
    NodoAST *nodo = crear_nodo_base(AST_AGREGARREGLA, linea);
    nodo->datos.logica_comando.expresion = expresion;
    return nodo;
}

NodoAST *crear_nodo_consultar(NodoAST *expresion, int linea)
{
    NodoAST *nodo = crear_nodo_base(AST_CONSULTAR_LOGICA, linea);
    nodo->datos.logica_comando.expresion = expresion;
    return nodo;
}

NodoAST *crear_nodo_limpiarlogica(int linea)
{
    NodoAST *nodo = crear_nodo_base(AST_LIMPIARLOGICA, linea);
    nodo->datos.logica_simple.dummy = 0;
    return nodo;
}

NodoAST *crear_nodo_listarhechos(int linea)
{
    NodoAST *nodo = crear_nodo_base(AST_LISTARHECHOS, linea);
    nodo->datos.logica_simple.dummy = 0;
    return nodo;
}

NodoAST *crear_nodo_listarreglas(int linea)
{
    NodoAST *nodo = crear_nodo_base(AST_LISTARREGLAS, linea);
    nodo->datos.logica_simple.dummy = 0;
    return nodo;
}

NodoAST *crear_nodo_encontrartodos(const char *variable, NodoAST *consulta,
                                   const char *resultado, int linea)
{
    NodoAST *nodo = crear_nodo_base(AST_ENCONTRARTODOS, linea);
    nodo->datos.encontrartodos.variable = variable ? strdup(variable) : NULL;
    nodo->datos.encontrartodos.consulta = consulta;
    nodo->datos.encontrartodos.resultado = resultado ? strdup(resultado) : NULL;
    return nodo;
}
...
void liberar_nodo(NodoAST* nodo) {
    ...
    switch (nodo->tipo) {
        ...
        case AST_CONSULTAR_LOGICA:
            if (nodo->datos.logica_comando.expresion)
            {
                liberar_nodo(nodo->datos.logica_comando.expresion);
            }
            break;
        case AST_ENCONTRARTODOS:
            if (nodo->datos.encontrartodos.variable)
                free(nodo->datos.encontrartodos.variable);
            if (nodo->datos.encontrartodos.consulta)
                liberar_nodo(nodo->datos.encontrartodos.consulta);
            if (nodo->datos.encontrartodos.resultado)
                free(nodo->datos.encontrartodos.resultado);
            break;
        case AST_LIMPIARLOGICA:
        case AST_LISTARHECHOS:
        case AST_LISTARREGLAS:
            // No tienen sub-nodos que liberar
            break;
        default:
            break;
    }
