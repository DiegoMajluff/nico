...
📌 Requieren VARIABLE TEXTO declarada. Escriben por referencia. Hora local del sistema (sin TZ/UTC en v2.1.0).
...
|------|-------------|-----|
| ENTERA | ±9.2×10¹⁸ | Contadores, índices |
...
| ENTERA SIN SIGNO | 0 a 18.4×10¹⁸ | IDs, tamaños |
...
| Comando | Sintaxis | Descripción |
|---------|----------|-------------|
| AGREGARHECHO | `AGREGARHECHO("predicado(args)")` | Agrega un hecho a la base de conocimiento |
| AGREGARREGLA | `AGREGARREGLA("cabeza SI cuerpo")` | Agrega una regla (también acepta `:-`) |
| CONSULTAR | `CONSULTAR("predicado(args)")` | Realiza una consulta al motor |
| LIMPIARLOGICA | `LIMPIARLOGICA` | Limpia toda la base de conocimiento |
| LISTARHECHOS | `LISTARHECHOS` | Muestra todos los hechos |
| LISTARREGLAS | `LISTARREGLAS` | Muestra todas las reglas |
| BUSCARTODOS | `BUSCARTODOS("variable", "consulta", $resultado)` | Recolecta todas las soluciones en una lista |

### 🔧 Built-ins del Motor

| Built-in | Sintaxis | Descripción |
|----------|----------|-------------|
| MIEMBRO | `MIEMBRO(X, Lista)` | Verifica si X es miembro de Lista |
| LONGITUD | `LONGITUD(Lista, N)` | Calcula la longitud de una lista |
| PRIMER | `PRIMER(Lista, X)` | Obtiene el primer elemento |
| ULTIMO | `ULTIMO(Lista, X)` | Obtiene el último elemento |
| JUNTAR | `JUNTAR(L1, L2, L3)` | Concatenación bidireccional (estilo APPEND) |
| REVERSA | `REVERSA(L1, L2)` | Invierte una lista (estilo REVERSE) |
| = | `X = valor` | Unificación directa |
...
BLOQUE PRINCIPAL
    ...
    // Recolectar todos los abuelos en una lista
...
Las variables del motor lógico se copian automáticamente a variables imperativas de Nico:
...
📚 **Documentación completa**: Ver [21-motor-logico.md](21-motor-logico.md) para ejemplos avanzados, built-ins detallados y consideraciones de rendimiento.

---

## 🛡️ Manejo de Errores (INTENTAR / ATRAPAR)

Nico permite capturar y manejar errores en tiempo de ejecución sin detener el programa, utilizando bloques `INTENTAR` y `ATRAPAR`.

### 📐 Sintaxis

```nico
INTENTAR
    // Código que podría generar un error (ej: división por cero, ALERTA)
ATRAPAR
    // Código que se ejecuta solo si ocurrió un error
    ESCRIBIR("Error: $ERROR") SALTO
    ESCRIBIR("Línea: $LINEA_ERROR") SALTO
    ESCRIBIR("Código: $CODIGO_ERROR") SALTO
FIN INTENTAR
```

### 🔑 Variables Mágicas en ATRAPAR

| Variable | Tipo | Descripción |
|----------|------|-------------|
| `$ERROR` | TEXTO | Mensaje descriptivo del error (ej: "División por cero") |
| `$LINEA_ERROR` | ENTERA | Número de línea donde ocurrió el fallo |
| `$CODIGO_ERROR` | ENTERA | Código numérico interno del error |

### 🎯 Ejemplo de Uso

```nico
PROGRAMA TestErrores
BLOQUE PRINCIPAL
    INTENTAR
        VARIABLE DECIMAL $resultado = 0.0
        CALCULAR EN $resultado = 10 / 0
        ESCRIBIR("Esta línea NO se imprimirá") SALTO
    ATRAPAR
        ESCRIBIR("Error capturado en línea $LINEA_ERROR: $ERROR") SALTO
    FIN INTENTAR
    
    ESCRIBIR("El programa continúa normalmente después del ATRAPAR") SALTO
FIN PRINCIPAL
FINAL
```

📌 **Nota**: Si no hay errores en el bloque `INTENTAR`, el bloque `ATRAPAR` se ignora por completo y la ejecución salta directamente a después de `FIN INTENTAR`.
...
Nico v2.1.0 usa un **Abstract Syntax Tree (AST)** en lugar de interpretación línea por línea:
...
|------|--------|-------|-----|
| ENTERA | 64-bit | ±9.2×10¹⁸ | Contadores, índices |
...
| ENTERA SIN SIGNO | 64-bit | 0 a 18.4×10¹⁸ | IDs, tamaños |
| DECIMAL SIN SIGNO | 64-bit | 0 a +1.7×10³⁰⁸ | Magnitudes físicas |

⚠️ **Validación de tipos SIN SIGNO:** Tanto `ENTERA SIN SIGNO` como `DECIMAL SIN SIGNO` rechazan valores negativos en tiempo de ejecución. Asignar un valor negativo a estos tipos produce un error de ejecución.
...
| MATRIZ 2D | `MATRIZ TIPO $nombre[filas][columnas]` | 64×64 | Arrays 2D (tableros, mapas) |
| MATRIZ 3D | `MATRIZ TIPO $nombre[x][y][z]` | 16×16×16 | Arrays 3D (vóxeles, cubos de datos) |

Todas soportan los tipos nativos (incluyendo `LOGICA`). La asignación se realiza con `ASIGNAR EN $mat[x][y] = valor` o `CALCULAR EN $mat[x][y] = expresión`.

⚠️ **Importante sobre la declaración de matrices:**
Nico requiere que cada matriz (2D o 3D) se declare en una línea independiente. **No se soporta la declaración múltiple con comas**.

```nico
// ❌ INCORRECTO (genera comportamiento inesperado)
MATRIZ ENTERA $a[10][10], $b[10][10]

// ✅ CORRECTO
MATRIZ ENTERA $a[10][10]
MATRIZ ENTERA $b[10][10]
```
...
> 📚 **Documentación validada con Nico v2.1.0**
> 
