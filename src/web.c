...
static void handle_csv_export(int fd, const char *path) {
    ...
    if (sqlite3_prepare_v2(ctx_bd.db, sql, -1, &stmt, 0) != SQLITE_OK) {
...
static void handle_csv_import(int fd, const char *path, const char *body, size_t bsize) {
    ...
    sqlite3_exec(ctx_bd.db, "BEGIN", 0, 0, 0);
    char cols_list[512] = "";
    char placeholders[256] = "";
    int col_count = 0;
    sqlite3_stmt *info;
    char qinfo[256]; snprintf(qinfo, sizeof(qinfo), "PRAGMA table_info(%s)", tbl);
    if (sqlite3_prepare_v2(ctx_bd.db, qinfo, -1, &info, 0) == SQLITE_OK) {
    ...
    if (col_count == 0) {
        free(data); sqlite3_exec(ctx_bd.db, "ROLLBACK", 0, 0, 0);
    ...
    while (line && *line && !error_msg) {
        ...
        sqlite3_stmt *stmt;
        if (sqlite3_prepare_v2(ctx_bd.db, insert_sql, -1, &stmt, 0) != SQLITE_OK) {
            error_msg = sqlite3_errmsg(ctx_bd.db); break;
        }
        int pos = 1, csv_idx = 0;
        char *tok = line;
        ...
        else if (rc != SQLITE_ROW) { error_msg = sqlite3_errmsg(ctx_bd.db); }
        sqlite3_finalize(stmt);
        line = next;
    }
    if (error_msg || imported == 0) sqlite3_exec(ctx_bd.db, "ROLLBACK", 0, 0, 0);
    else sqlite3_exec(ctx_bd.db, "COMMIT", 0, 0, 0);
    free(data);
    char json[512];
    if (error_msg) {
...
static void handle_foreign_keys(int fd, const char *path) {
    ...
    if (sqlite3_prepare_v2(ctx_bd.db, q, -1, &s, 0) == SQLITE_OK) {
        char resp[4096] = "[";
        size_t l = 1;
        int first = 1;
...
static void* handle_client(void *arg) {
    ...
    if (strcmp(path, "/admin") == 0) {
    ...
    else if (strcmp(path, "/api/tables") == 0 && ctx_bd.db != NULL) {
        sqlite3_stmt *s;
        if(sqlite3_prepare_v2(ctx_bd.db, "SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name;", -1, &s, 0)==SQLITE_OK) {
        ...
        } else { strcpy(resp,"[]"); status=200; }
    }
    else if (strncmp(path, "/api/schema/", 12) == 0 && ctx_bd.db != NULL) {
    ...
    else if (strncmp(path, "/api/foreign-keys/", 18) == 0 && ctx_bd.db != NULL) {
        handle_foreign_keys(fd, path);
        if(body_alloc) { free(body); }
        return NULL;
    }
    else if (strcmp(path, "/api/query") == 0 && strcmp(method, "POST") == 0 && ctx_bd.db != NULL && body) {
        char *sql=body; sql[strcspn(sql,"\r\n")]='\0';
        sqlite3_stmt *s;
        if(sqlite3_prepare_v2(ctx_bd.db,sql,-1,&s,0)==SQLITE_OK) {
            resp[0]='['; size_t l=1; int f=1;
            while(sqlite3_step(s)==SQLITE_ROW) {
    ...
    else if (strcmp(path, "/api/exec") == 0 && strcmp(method, "POST") == 0 && ctx_bd.db != NULL && body) {
        char *sql=body; sql[strcspn(sql,"\r\n")]='\0';
        char *err=NULL;
        if(sqlite3_exec(ctx_bd.db,sql,NULL,NULL,&err)==SQLITE_OK) {
...
int cmd_iniciarserver(const char *linea, CtxBloque *ctx, int linea_actual) {
    (void)linea_actual;
    if (server_running) { printf("⚠️ El servidor ya está en ejecución.\n"); ctx->linea_num++; return 0; }
    const char *p = strchr(linea, '(');
    if (!p) { ctx->linea_num++; return 0; }
    p++; while (isspace(*p)) p++;
    int port = atoi(p);
    if (port <= 0) port = 8080;
    server_running = 1;
    pthread_t tid;
    int *pp = malloc(sizeof(int)); *pp = port;
    pthread_create(&tid, NULL, server_loop, pp);
    pthread_detach(tid);
    ctx->linea_num++; return 0;
}

int cmd_cerrarserver(const char *linea, CtxBloque *ctx, int linea_actual) {
    (void)linea; (void)linea_actual;
    if (!server_running || listen_fd_global == -1) { printf("⚠️ No hay servidor web en ejecución.\n"); ctx->linea_num++; return 0; }
