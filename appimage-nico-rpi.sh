...
VERSION="2.1.0"
APPDIR="$PROJECT_DIR/AppDir-aarch64"
OUTPUT=""
...
usage() {
    cat <<'USAGE'
Generador de AppImage para Nico + Nico IDE en Raspberry Pi aarch64

Uso:
  ./appimage-nico-rpi.sh [opciones]
...
Ejemplos:
  ./appimage-nico-rpi.sh
  ./appimage-nico-rpi.sh --version 2.1.0
USAGE
}
...
file "$NICOC_BIN" | grep -qi "aarch64" || warn "El intérprete no parece aarch64."
file "$IDE_BIN_PATH" | grep -qi "aarch64" || warn "El IDE no parece aarch64."
